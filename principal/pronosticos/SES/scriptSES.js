...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    var demanda = document
      ...
      .value.split(",")
      .map(Number);
    var alfa = parseFloat(document.getElementById("alfa").value);

    // Función para calcular el suavizado exponencial simple
    function calcularSuavizadoExponencialSimple(demanda, alfa) {
      var pronostico = [demanda[0]];
      for (var i = 1; i < demanda.length; i++) {
        pronostico.push(alfa * demanda[i - 1] + (1 - alfa) * pronostico[i - 1]);
      }
      return pronostico;
    }

    // Calcular el pronóstico
    var pronostico = calcularSuavizadoExponencialSimple(demanda, alfa);
...
html2canvas(container, {
  ...
  var storageRef = firebase.storage().ref('imagesSES/' + nombreImagen + '.png');
